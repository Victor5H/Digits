package com.github.Victor5H;

import java.util.LinkedList;
import java.util.List;

public class Digits {
    static public void aboutIt() {
        System.out.println("This is a public class with static methods to perform some operation on digits");
        System.out.println("Methods are: ");
        System.out.println("");
    }

    static public int numberOfDigits(long num) {
        int count = 0;
        if (num == 0) return 1;
        while (num > 0) {
    ...
    public static long sumOfDigitsUnit(long num) {  // calculate sum of digits till results into unit digit
        long temp, sum = 0;
        while (num > 0) {
    ...
    static public void fibonacciTo(long num) {
        long sum = 0L, a, b = 1L;
        while (num > 0) {
    ...
    static public boolean isArmstrong(long num) {
    ...
    static public long sumOfDigits(long num) {
        long sum = 0, dig;
        while (num > 0) {
            dig = num % 10;
            sum += dig;
    ...
    static public boolean isEven(long num) {
        if (num % 2 == 0)
            return true;
        else
            return false;
    ...
    static public boolean isOdd(long num) {
        if (num % 2 != 0)
            return true;
        else
            return false;
    ...
    static public boolean isComposite(long num) {
        if (num <= 1) {
            return true;
        }
        for (int i = 2; i < num; i++) {
    ...
    public static boolean isTwinPrime(long n1, long n2) {
        long diff;
    ...
    public static boolean isPerfectNum(long num) {  // sum of factors = number itself
        long sum = 0;
        for (int i = 1; i < num; i++) {
    ...
    public static long euclidHCF(long n1, long n2) {
        long r = 0;
        while (n2 % n1 != 0) {
            r = n2 % n1;
            n2 = n1;
    ...
    public static int[] DecToBin(long num) {
        int rem = 0;
        int[] bin = new int[64];
        int index = 0;
        while (num > 0) {
    ...
    public static List<Integer> PrimeFromTO(int from, int to) {
